...
module.exports = function (app) {
    ...
    app.get('/stats', (req, res) => {
        if (req.isAuthenticated()) {
            ...
            let query2 = 'SELECT s.stkh_name, c.comp_Name, se.quantity as fsale, t.quantity as sold FROM stockholder s, company c, selling se, transactions t WHERE se.selling_id = t.sell_id and t.compID = c.comp_id and t.stkh_ID = s.stkh_id';
            let viewName = 'stats.hbs';
            getMultipleData(query1, query2, req, res, viewName);
...
async function getMultipleDataFromDB(query1, query2, req, res, viewName) {
    let data1 = await null;
    let data2 = await null;
    return new Promise(function (resolve, reject) {

        pool.query('BEGIN');
        JSON.stringify(pool.query(query1, async (err, result) => {
            if (err) {
                res.json({
                    "Response": "Error fetching data.",
                    "Error": err
        ...
        JSON.stringify(pool.query(query2, async (err, result) => {
            if (err) {
                res.json({
                    "Response": "Error fetching data.",
                    "Error": err
...
async function getMultipleData(query1, query2, req, res, viewName) {
    await getMultipleDataFromDB(query1, query2, req, res, viewName);
}
