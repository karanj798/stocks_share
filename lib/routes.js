...
async function updateStockPrice(){
	let queryWrite = `SELECT company.comp_name FROM company`;
	let response;
	var comp_query;
	
	//get total number companies and ticker
	request.get({
		url: 'http://localhost:3000/api/company/comp_ticker'
	}, (err, res, body_call) => {
		if (err) {
		...
		if(res){
			...
			console.log(comp_query[1]['comp_ticker']);
			...
			for (var i = 0; i < comp_query.length; i++) {
				if (comp_query[i]['comp_ticker'] != null) {
					request.post({
						url: `http://localhost:3000/api/stocks/${comp_query[i]['comp_ticker']}/TIME_SERIES_DAILY`
					}, (err, res, bodyy) => {
						if (err) {
							return console.log(err);
						}
